...
SIR.threshold <- function(Y, X, H = 10, lambda = 0, thresholding = "hard") {
    ...
    res = list(beta = b.estim, M1_th = M1_th, eig.val = eig.values, eig.vect = eig.vectors, n = n, p = p,H=H, nb.zeros = nb.zeros, list.relevant.variables = list.relevant.variables, cos.squared = cos.squared,lambda=lambda,thresholding=thresholding)
