...
plot.SIR.threshold.opt.R = function(x,...){
    ...
    if (length(which(x$vect.nb.zeros == x$fit_bp$breakpoints)) > 0) {
        ...
        plot(sort(x$indice.0, decreasing = FALSE), xlab = "variable i", ylab = expression("Indices of" ~ lambda ~ "from which the variable is not selected"))
        # ligne verticale pour montrer la rupture
        abline(v = x$fit_bp$breakpoints + 0.5, col = 6, lwd = 3)
        title(paste("Choosing the optimal lambda"))
        
    }
    else {
        par(mfrow = c(1, 2))
        # affichage des indices triés
        plot(sort(x$indice.0, decreasing = FALSE), xlab = "variable p_i", ylab = expression("Indices of" ~ lambda ~ "from which the variable i is not selected"))
        # ligne verticale pour montrer la rupture
    ...
    plot(x$lambdas, 1 - x$vect.nb.zeros / x$p, ylim = c(0, 1.3), xlim = c(0, max(x$lambdas)), xlab = expression(lambda), col = 3, type = "l", ylab = "", lwd = 3)
    
    # Affichage de l'évolution du cos carré entre SIR et SParseSIR en fonction des valeurs de lambda
    points(x$lambdas, x$vect.cosca, type = "l", ylab = "", xlab = "lambda", lwd = 3, col = "black")
